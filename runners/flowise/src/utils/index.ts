...
export const buildLangchain = async (
  ...
  for (let i = 0; i < startingNodeIds.length; i += 1) {
    ...
    exploredNode[startingNodeIds[i]] = {
      remainingLoop: maxLoop,
      lastSeenDepth: 0,
    };
  }
  ...
  while (nodeQueue.length) {
    ...
    try {
      const nodeInstanceFilePath = componentNodes[reactFlowNode.data.name]
        .filePath as string;
      ...
      logger.debug(
        `[server]: Initializing ${reactFlowNode.data.label} (${reactFlowNode.data.id})`
      );
      ...
      logger.debug(
        `[server]: Finished initializing ${reactFlowNode.data.label} (${reactFlowNode.data.id})`
      );
    } catch (e: any) {
      logger.error(e);
      throw new Error(e);
    ...
    const sameDepthNodeIds = Object.keys(depthQueue).filter(
      (key) => depthQueue[key] === nextDepth
    );
    ...
    for (let i = 0; i < neighbourNodeIds.length; i += 1) {
      ...
      if (Object.prototype.hasOwnProperty.call(exploredNode, neighNodeId)) {
        ...
        exploredNode[neighNodeId] = {
          remainingLoop: remainingLoopMinusOne,
          lastSeenDepth: nextDepth,
        };
...
export const clearAllSessionMemory = async (
  ...
  for (const node of reactFlowNodes) {
    if (node.data.category !== "Memory" && node.data.type !== "OpenAIAssistant")
      continue;
    const nodeInstanceFilePath = componentNodes[node.data.name]
      .filePath as string;
    ...
    if (newNodeInstance.clearSessionMemory) {
      await newNodeInstance?.clearSessionMemory(node.data, {
        chatId,
        appDataSource,
        databaseEntities,
        logger,
...
export const getVariableValue = (
  ...
  while (startIdx < endIdx) {
    ...
    if (
      ...
      const executedNode = reactFlowNodes.find(
        (nd) => nd.id === variableNodeId
      );
      if (executedNode) {
    ...
    for (const key in paramsObj) {
      ...
      if (Array.isArray(paramValue)) {
        ...
        for (const param of paramValue) {
          const resolvedInstance = getVariableValue(
            param,
            reactFlowNodes,
            question,
            chatHistory
        ...
        const isAcceptVariable =
          reactFlowNodeData.inputParams.find((param) => param.name === key)
  ...
  const getParamValues = (paramsObj: ICommonObject) => {
    for (const config in overrideConfig) {
      ...
      if (
        ...
        typeof overrideConfig[config] === "object"
      ) {
 ...
 * @returns {boolean}
 */
export const isStartNodeDependOnInput = (
...
export const getAPIKeyPath = (): string => {
  return process.env.APIKEY_PATH
    ? path.join(process.env.APIKEY_PATH, "api.json")
...
export const updateAPIKey = async (
  ...
  await fs.promises.writeFile(
    getAPIKeyPath(),
    JSON.stringify(existingAPIKeys),
    "utf8"
  );
 ...
 */
export const replaceAllAPIKeys = async (
 ...
 */
export const findAvailableConfigs = (
  reactFlowNodes: IReactFlowNode[],
    ...
    for (const inputParam of flowNode.data.inputParams) {
      ...
      if (inputParam.type === "file") {
        ...
        for (const name of inputParam.credentialNames ?? []) {
          if (
            Object.prototype.hasOwnProperty.call(componentCredentials, name)
          ) {
      ...
      if (
        !configs.some(
          (config) => JSON.stringify(config) === JSON.stringify(obj)
        )
      ) {
        configs.push(obj);
      }
 ...
 * @returns {boolean}
 */
export const isFlowValidForStream = (
  reactFlowNodes: IReactFlowNode[],
 ...
 * @returns {Credential}
 */
export const transformToCredentialEntity = async (
...
export const checkMemorySessionId = (instance: any, chatId: string): string => {
  if (
    instance.memory &&
    instance.memory.isSessionIdUsingChatMessageId &&
    chatId
  ) {
  ...
  return instance.memory
    ? instance.memory.sessionId ?? instance.memory.chatHistory.sessionId
    : undefined;
};
